    ...
    internal class FlashlightItemPatch
    {
        ...
        private static void StunFlash(ref FlashlightItem __instance)
        {
                ...
                && !__instance.itemProperties.itemName.Equals("Laser pointer"))
            {
                ...
                {
                    addon.isFlashing = false;
                }
                else if (__instance.playerHeldBy == GameNetworkManager.Instance.localPlayerController
                    ...
                    && !addon.isFlashing)
                {
                    ...
                    foreach (EnemyAI enemy in blindableEnemies.Where(e => e.enemyType != null && e.enemyType.canBeStunned && e.eye != null))
                    {
                        ...
                        if (lensValue != null)
                        {
                            ...
                            if (MeetConditions(enemy, ref flashlightItem, ref addon, lightAngle, angle, distance))
                            {
                                __instance.StartCoroutine(StunCoroutine(flashlightItem, addon, enemy, flashDuration, lightAngle, angle, distance, batteryConsumption, stunDuration));
                                break;
                            }
                        }
                        else
                    ...
                    if (lensValuePlayer != null && !addon.isFlashing)
                    {
                        foreach (PlayerControllerB player in StartOfRound.Instance.allPlayerScripts.Where(p => p.isPlayerControlled && p != flashlightItem.playerHeldBy))
                        {
                            ...
                            if (MeetConditions(player, ref flashlightItem, ref addon, lightAngle, angle, distance))
                            {
                                __instance.StartCoroutine(StunCoroutine(flashlightItem, addon, player, flashDuration, lightAngle, angle, distance, batteryConsumption));
                                break;
                            }
                        }
                    }

                    if (__instance.flashlightBulb.spotAngle != addon.maxSpotAngle)
                    {
                        __instance.flashlightBulb.spotAngle = addon.maxSpotAngle;
                    }
                }
            }
        }

        private static IEnumerator StunCoroutine<T>(FlashlightItem flashlightItem, Addon addon, T entity, float flashDuration, float lightAngle, float angle, int distance, float batteryConsumption, float stunDuration = 0f)
        {
            float timePassed = 0f;
            float minSpotAngle = addon.maxSpotAngle / 3f;

            while (MeetConditions(entity, ref flashlightItem, ref addon, lightAngle, angle, distance))
            {
                yield return new WaitForSeconds(0.1f);
                timePassed += 0.1f;
                flashlightItem.flashlightBulb.spotAngle = Mathf.Lerp(addon.maxSpotAngle, minSpotAngle, timePassed / flashDuration);
            ...
            if (addon.isFlashing)
            {
                flashlightItem.flashlightBulb.spotAngle = addon.maxSpotAngle * 2;
            ...
            flashlightItem.flashlightBulb.spotAngle = addon.maxSpotAngle;
        }

        private static bool MeetConditions<T>(T entity, ref FlashlightItem flashlightItem, ref Addon addon, float lightAngle, float angle, int distance)
        {
